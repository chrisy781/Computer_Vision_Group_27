...
## 6. Discussion
There are many different areas that could be changed in order to improve results. The first, and according to some researchers, most promising method would be the use of another model than YoloV4. Those other models (e.g. SNIP or SNIPER) as described in the introduction could be implemented and used to obtain possible better results. However since these networks are not widely adapted, these are hard to implement ourselves. YoloV4 itself is also complex in example because of the many different techniques it uses to come to the state-of-the-art performances on the ImageNet and COCO datasets. At the same time a lot of info is publicly available concerning the use and implementation of YoloV4 which makes it very easy to use and implement. That being said our project has shown that it looks like YoloV4 has even more room for improvement. More specifically in the field of small object the detection such as golf balls. A combination of dynamic tiling (where we have varying size of tiles) and more and better generalized training data might be the next step in order to detect every golf ball on the driving range. 
